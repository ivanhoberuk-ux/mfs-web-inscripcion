CREATE OR REPLACE FUNCTION public.torneo_programar(
  p_edicion_id uuid,
  p_reprogramar_todo boolean DEFAULT true,
  p_max_dia_pueblo integer DEFAULT 4,
  p_descanso_min integer DEFAULT 30
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_max_dia_pueblo integer := greatest(coalesce(p_max_dia_pueblo,4),1);
...
BEGIN
  ...
  LOOP
    ...
    IF NOT v_ok THEN
      ...
      v_pendientes := v_pendientes || jsonb_build_object(
        ...
        'zona', v_p.zona,
        'ronda', v_p.ronda,
        'motivo', CASE WHEN v_cabe THEN 'Sin hueco libre (cancha ocupada, tope de partidos por día del pueblo o descanso mínimo)'
  ...
  SELECT jsonb_agg(x) INTO v_resumen FROM (
    ...
    GROUP BY td.id, td.emoji, td.nombre, td.duracion_min, td.buffer_min, td.orden
    ORDER BY td.orden
  ) s;
  ...
  RETURN jsonb_build_object(
    ...
    'max_dia_pueblo', v_max_dia_pueblo,
    'descanso_min', greatest(coalesce(p_descanso_min,0),0),

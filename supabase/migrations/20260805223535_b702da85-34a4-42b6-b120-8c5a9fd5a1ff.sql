...
DECLARE
  ...
  v_tz interval := interval '-3 hours';  -- Paraguay: UTC-3 fijo
  v_ini timestamptz;
  v_fin timestamptz;
...
BEGIN
  ...
  LOOP
    ...
    FOR v_bloque IN SELECT * FROM torneo_bloques WHERE edicion_id = p_edicion_id ORDER BY fecha, hora_inicio LOOP
      v_ini := (v_bloque.fecha + v_bloque.hora_inicio) AT TIME ZONE v_tz;
      v_fin := (v_bloque.fecha + v_bloque.hora_fin) AT TIME ZONE v_tz;

      IF v_ini + v_dur <= v_fin THEN
        v_cabe := true;
      END IF;
      ...
      IF v_max_dia_pueblo < 9999 AND EXISTS (
        ...
        WHERE (
          ...
          WHERE o.inicio IS NOT NULL
            AND (o.inicio AT TIME ZONE v_tz)::date = v_bloque.fecha
